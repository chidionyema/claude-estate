...
**We are selling this. Buy the mature platform. Do not stitch one.** One platform, one name:
`~/dev/code/idp`. `prospector`/`hermes-v2` etc. are products, not the platform — nothing here says
a product must live inside `idp`. What the rule binds is the layers underneath (model routing,
traces/audit, identity, secrets, scheduling, catalog, CI): one of each, in `idp`, onboarded not
duplicated. Founder's verbatim words and full rationale: `~/AGENTS-FULL.md`.
...
## THE MEDIC LAW AND CONSENT (founder 2026-09-09)

1. The medic/local-admin agent never thinks through the cluster LLM router; it runs on local
   silicon (Ollama) or a direct external line, so it can still think when the cluster is down.
2. Consent for any production/cluster/config change comes only from the founder's own explicit
   plain words — never from an options/choice UI selection. Full text: `~/AGENTS-FULL.md`.

# THE EXECUTION BOUNDARY (founder 2026-09-13)

Nothing an agent runs may hold a turn open. 60-second ceiling, enforced by a daemon outside the
agent's process tree, not by the agent itself.

- Run: `bin/idp-exec <command...>` (returns a job id; read later with `--read <job-id>`).
- Longer than 60s: `dispatch_job`, never `timeout`/`&`/a raised ceiling — all refused at the door.
- Boundary status is UNKNOWN, not enforced-closed, until a separate `_idp_executor` uid ships
  (waits on the founder, LAW 54). Full text and incident history: `~/AGENTS-FULL.md`.
