...
# The executor's memory across sessions (founder 2026-08-27: "every ticket it tackles it needs
# to improve its ... knowledge of the stack ... and retain it thru sessions"). Prepended to every
# plan; `lesson` on a pi_execute call appends one line to its Lessons section after the run.
PRIMER = os.environ.get("PI_BRIDGE_PRIMER") or os.path.join(os.path.dirname(os.path.abspath(__file__)), "executor-primer.md")
LESSONS_HEADING = "## Lessons"


def load_primer(path: str = PRIMER) -> str:
    try:
...
def compose_prompt(plan: str, primer: str) -> str:
    head = ("You are the EXECUTOR. Implement the following approved plan exactly as written.\n"
    ...
    if primer:
        head += "=== ESTATE PRIMER (what earlier jobs learned; it binds) ===\n" + primer + "\n\n"
    return head + "=== PLAN ===\n" + plan


def add_lesson(lesson: str, path: str = PRIMER, day: str | None = None) -> bool:
    """Append one lesson line under the Lessons heading; False when it is already there or
    the primer has no such heading. One line, no newlines, dated."""
    lesson = " ".join((lesson or "").split())
    if not lesson:
        return False
    text = load_primer(path)
    if LESSONS_HEADING not in text:
        return False
    if lesson in text:
        return False
    day = day or time.strftime("%Y-%m-%d")
    with open(path, "a", encoding="utf-8") as f:
        f.write(f"- {day} {lesson}\n")
    return True
...
def tool_pi_execute(a: dict) -> str:
    ...
    primer = "" if a.get("no_primer") else load_primer()
    argv.append(compose_prompt(plan, primer))
    ...
    if lesson:
        parts.append("lesson: " + ("recorded in the primer" if add_lesson(lesson) else "already in the primer (or no Lessons heading)"))
    parts.append(f"primer: {len([l for l in primer.splitlines() if l.startswith('- ')])} lines carried" if primer else "primer: off")
    if rc == 124:
...
TOOLS = [
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "lesson": {"type": "string", "description": "One line the NEXT job should know (what the plan got wrong, what the executor had to look up). Appended to the primer's Lessons after the run."},
                "no_primer": {"type": "boolean", "description": "Skip the estate primer. Default false."},
